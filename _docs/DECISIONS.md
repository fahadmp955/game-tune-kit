...
| **Port-Adapter-Resolver Pattern** | Decided | Architecture | External push gateways are decoupled from business logic via `NotificationPushPort`. `PushAdapterResolver` dynamically routes between `FcmPushAdapter`, `ApnsPushAdapter`, `WebPushAdapter`, and `MockPushAdapter` based on platform and environment. |
...
| **100% Dynamic REST Dashboard** | Decided | Architecture | Eliminate static mock fallback arrays in the PNS Studio frontend. All game tenants, player cohorts, historical campaigns, and delivery KPI metrics are fetched and mutated purely via live REST APIs. |
| **Cached Reach Column on Segments** | Decided | Database / Perf | Store `cachedReach` on `Segment` entity in PostgreSQL. Evaluated on campaign dispatch, segment creation, or on-demand probe, eliminating expensive full-table player scans during interactive browsing. |
| **Campaign Preset Selector in Composer** | Decided | UI / UX | Provide a template selector in Campaign Composer so game operations teams can choose from saved/historical campaigns to auto-populate notification title, body, cohort, and deep-link screen. |
