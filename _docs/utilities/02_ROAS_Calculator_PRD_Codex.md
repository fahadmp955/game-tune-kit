# GameTuneKit Utility #02 — ROAS Calculator

## Part A — Product / Method Research

### Product question

> **What return on ad spend does a campaign generate at D7/D30/D90/D180/D365, and when does it cross break-even?**
...
|---|---|---|---|
| Ad spend | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Paid installs | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Revenue at one or more horizons | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional organic multiplier | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
...
- `ROAS% = ROAS × 100`
- `Revenue per paid install = Revenue / Paid Installs`
...
- Revenue per install
- Break-even status
- Incremental revenue required to break even
...
- Verify `ROAS = Attributed Revenue / Ad Spend` with at least one hand-calculated deterministic vector.
- Verify `ROAS% = ROAS × 100` with at least one hand-calculated deterministic vector.
- Verify `Revenue per paid install = Revenue / Paid Installs` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #02: ROAS Calculator
...
- Ad spend
- Paid installs
- Revenue at one or more horizons
- Optional organic multiplier
