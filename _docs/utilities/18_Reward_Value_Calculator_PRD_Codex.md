# GameTuneKit Utility #18 — Reward Value Calculator

## Part A — Product / Method Research

### Product question

> **What is the normalized economic value of a reward bundle containing multiple currencies/items?**
...
|---|---|---|---|
| Reward components | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Quantity per component | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Value anchor per component | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional probability | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
...
- `ComponentValue = Quantity × UnitValue × Probability`
- `BundleValue = sum ComponentValue`
- `ValueShare_i = ComponentValue / BundleValue`
- `SKUEquivalent = BundleValue / ReferenceSKUValue`
...
- Verify `ComponentValue = Quantity × UnitValue × Probability` with at least one hand-calculated deterministic vector.
- Verify `BundleValue = sum ComponentValue` with at least one hand-calculated deterministic vector.
- Verify `ValueShare_i = ComponentValue / BundleValue` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #18: Reward Value Calculator
