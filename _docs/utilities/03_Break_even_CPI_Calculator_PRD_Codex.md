# GameTuneKit Utility #03 — Break-even CPI Calculator

## Part A — Product / Method Research

### Product question

> **What is the maximum CPI/CAC I can pay for an install and still hit my target return?**
...
|---|---|---|---|
| LTV or target-horizon value per acquired user | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Organic multiplier / K-factor | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Target ROAS | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional platform/store/variable margin | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `NetUserValue = UserValue × VariableMargin`
- `EffectiveUsersPerPaidInstall = 1 + OrganicMultiplier`
- `BreakEvenCPI = NetUserValue × EffectiveUsersPerPaidInstall / TargetROAS`
...
- Break-even CPI
- CPI at target ROAS
- Headroom vs current CPI
- Effective value per paid install
...
- Verify `NetUserValue = UserValue × VariableMargin` with at least one hand-calculated deterministic vector.
- Verify `EffectiveUsersPerPaidInstall = 1 + OrganicMultiplier` with at least one hand-calculated deterministic vector.
- Verify `BreakEvenCPI = NetUserValue × EffectiveUsersPerPaidInstall / TargetROAS` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #03: Break-even CPI Calculator
...
- LTV or target-horizon value per acquired user
- Organic multiplier / K-factor
- Target ROAS
- Optional platform/store/variable margin
