# GameTuneKit Utility #32 — DAU / MAU Stickiness Calculator

## Part A — Product / Method Research

### Product question

> **How frequently does the average monthly active user return on a typical day?**
...
| MAU | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional WAU | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Period/date label | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
...
- `Stickiness = DAU / MAU`
- `StickinessPercent = Stickiness×100`
- `WAUStickiness = DAU/WAU when supplied`
...
- DAU/MAU stickiness
- DAU/WAU
- Implied active days/month = 30×DAU/MAU
...
- Verify `Stickiness = DAU / MAU` with at least one hand-calculated deterministic vector.
- Verify `StickinessPercent = Stickiness×100` with at least one hand-calculated deterministic vector.
- Verify `WAUStickiness = DAU/WAU when supplied` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #32: DAU / MAU Stickiness Calculator
