# GameTuneKit Utility #09 — IAP Pack Value Calculator

## Part A — Product / Method Research

### Product question

> **How much value does each IAP pack deliver, and are larger packs consistently more attractive than smaller packs?**
...
|---|---|---|---|
| Pack price | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Primary currency amount | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Bonus currency | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional included item values | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Reference/base pack | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `TotalCurrency = BaseCurrency + BonusCurrency`
- `CurrencyPerMoney = TotalCurrency / Price`
- `MoneyPer1000Currency = Price / TotalCurrency × 1000`
- `ValueIndex = CurrencyPerMoney / ReferenceCurrencyPerMoney`
- `EffectiveDiscount = 1 - ReferenceMoneyPerCurrency / PackMoneyPerCurrency`
...
- Currency per currency-unit of money
- Cost per 1,000 currency
- Value index
- Effective discount vs reference
- Pack ladder consistency
...
- Verify `TotalCurrency = BaseCurrency + BonusCurrency` with at least one hand-calculated deterministic vector.
- Verify `CurrencyPerMoney = TotalCurrency / Price` with at least one hand-calculated deterministic vector.
- Verify `MoneyPer1000Currency = Price / TotalCurrency × 1000` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #09: IAP Pack Value Calculator
