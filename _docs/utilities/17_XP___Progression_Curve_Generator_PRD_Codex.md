# GameTuneKit Utility #17 — XP / Progression Curve Generator

## Part A — Product / Method Research

### Product question

> **How much XP should each level require, and what cumulative progression/time curve does that create?**
...
|---|---|---|---|
| Number of levels | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Base XP | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Curve type | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Growth parameter | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional XP earned per minute/day | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `Linear: XP_n=BaseXP + Step×(n-1)`
- `Exponential: XP_n=BaseXP×Growth^(n-1)`
- `Power: XP_n=BaseXP×n^Exponent`
- `CumulativeXP_n=sum XP_1..XP_n`
- `TimeToLevel=CumulativeXP/XPEarningRate when supplied`
...
- Verify `Linear: XP_n=BaseXP + Step×(n-1)` with at least one hand-calculated deterministic vector.
- Verify `Exponential: XP_n=BaseXP×Growth^(n-1)` with at least one hand-calculated deterministic vector.
- Verify `Power: XP_n=BaseXP×n^Exponent` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #17: XP / Progression Curve Generator
...
- Curve type
- Growth parameter
- Optional XP earned per minute/day
