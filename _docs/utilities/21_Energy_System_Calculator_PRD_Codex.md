# GameTuneKit Utility #21 — Energy System Calculator

## Part A — Product / Method Research

### Product question

> **How restrictive is an energy system, how many sessions/actions does it permit, and what is the time/value impact of refills?**
...
|---|---|---|---|
| Energy cap | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Starting energy | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Energy cost/action | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Regen amount | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Regen interval | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Average action duration | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Refill amount | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Refill price | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `ActionsFromCap=floor(Cap/CostPerAction)`
- `RegenPerHour=RegenAmount×60/RegenIntervalMinutes`
- `HoursToFull=(Cap-CurrentEnergy)/RegenPerHour`
- `ActionsPerDayFromRegen=24×RegenPerHour/CostPerAction`
- `RefillActions=RefillAmount/CostPerAction`
- `CostPerExtraAction=RefillPrice/RefillActions`
...
- Actions per full bar
- Time to full
- Regen actions/day
- Playtime per bar
- Refill actions
- Cost per extra action
...
- Verify `ActionsFromCap=floor(Cap/CostPerAction)` with at least one hand-calculated deterministic vector.
- Verify `RegenPerHour=RegenAmount×60/RegenIntervalMinutes` with at least one hand-calculated deterministic vector.
- Verify `HoursToFull=(Cap-CurrentEnergy)/RegenPerHour` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #21: Energy System Calculator
