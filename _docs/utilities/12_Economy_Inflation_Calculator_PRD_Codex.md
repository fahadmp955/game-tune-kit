# GameTuneKit Utility #12 — Economy Inflation Calculator

## Part A — Product / Method Research

### Product question

> **Is currency entering the economy faster than it leaves, and how quickly does the circulating balance inflate or deflate?**
...
|---|---|---|---|
| Starting currency supply | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Daily currency sources | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Daily currency sinks | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Active players | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Projection days | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional source/sink growth rates | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `NetFlowDay = SourcesDay - SinksDay`
- `SupplyDay = SupplyPrevious + NetFlowDay`
- `InflationRate = (EndingSupply / StartingSupply) - 1`
- `NetFlowPerDAU = NetFlowDay / DAU`
...
- Net daily flow
- Ending supply
- Inflation/deflation percentage
- Net flow per DAU
- Source/sink ratio
...
- Verify `NetFlowDay = SourcesDay - SinksDay` with at least one hand-calculated deterministic vector.
- Verify `SupplyDay = SupplyPrevious + NetFlowDay` with at least one hand-calculated deterministic vector.
- Verify `InflationRate = (EndingSupply / StartingSupply) - 1` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #12: Economy Inflation Calculator
...
- Starting currency supply
- Daily currency sources
- Daily currency sinks
- Active players
- Projection days
- Optional source/sink growth rates
