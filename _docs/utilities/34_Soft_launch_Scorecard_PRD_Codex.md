# GameTuneKit Utility #34 — Soft-launch Scorecard

## Part A — Product / Method Research

### Product question

> **How does a soft-launch build perform against a set of explicit studio targets without relying on hidden industry benchmarks?**
...
|---|---|---|---|
| Metric rows: name, actual value, target value, direction higher/lower-is-better, weight | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional category | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `For higher-is-better: Score_i=min(1,Actual/Target)`
- `For lower-is-better: Score_i=min(1,Target/Actual), safely handle zero`
- `WeightedScore=sum(Score_i×Weight_i)/sum(Weight_i)×100`
...
- Overall target-attainment score
- Metric pass/fail
- Weighted contribution
- Strongest/weakest target gaps
...
- Verify `For higher-is-better: Score_i=min(1,Actual/Target)` with at least one hand-calculated deterministic vector.
- Verify `For lower-is-better: Score_i=min(1,Target/Actual), safely handle zero` with at least one hand-calculated deterministic vector.
- Verify `WeightedScore=sum(Score_i×Weight_i)/sum(Weight_i)×100` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #34: Soft-launch Scorecard
...
- Metric rows: name, actual value, target value, direction higher/lower-is-better, weight
- Optional category
