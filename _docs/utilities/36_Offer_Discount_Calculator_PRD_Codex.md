# GameTuneKit Utility #36 — Offer Discount Calculator

## Part A — Product / Method Research

### Product question

> **What is the real discount/value uplift of an offer compared with its normal component prices?**
...
|---|---|---|---|
| Offer price | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Components with normal standalone value | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional bonus components | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional reference SKU | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `ReferenceValue=sum component standalone values`
- `DiscountAmount=ReferenceValue-OfferPrice`
- `DiscountPercent=1-OfferPrice/ReferenceValue`
- `ValueMultiplier=ReferenceValue/OfferPrice`
- `BonusShare=BonusValue/ReferenceValue`
...
- Verify `ReferenceValue=sum component standalone values` with at least one hand-calculated deterministic vector.
- Verify `DiscountAmount=ReferenceValue-OfferPrice` with at least one hand-calculated deterministic vector.
- Verify `DiscountPercent=1-OfferPrice/ReferenceValue` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #36: Offer Discount Calculator
...
- Offer price
- Components with normal standalone value
- Optional bonus components
- Optional reference SKU
