# GameTuneKit Utility #07 — ARPDAU / ARPPU Calculator

## Part A — Product / Method Research

### Product question

> **What are my daily revenue-per-user and revenue-per-payer economics?**
...
| Daily revenue | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Daily paying users | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional ad revenue | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional IAP revenue | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
...
- `ARPPU = IAPRevenue / PayingUsers`
- `PayerConversion = PayingUsers / DAU`
- `AdARPDAU = AdRevenue / DAU`
- `IAPARPDAU = IAPRevenue / DAU`
...
- Verify `ARPDAU = DailyRevenue / DAU` with at least one hand-calculated deterministic vector.
- Verify `ARPPU = IAPRevenue / PayingUsers` with at least one hand-calculated deterministic vector.
- Verify `PayerConversion = PayingUsers / DAU` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #07: ARPDAU / ARPPU Calculator
