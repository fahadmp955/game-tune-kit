# GameTuneKit Utility #08 — PPP Price Calculator

## Part A — Product / Method Research

### Product question

> **What local price preserves comparable purchasing-power burden across markets without blindly applying FX conversion?**
...
|---|---|---|---|
| Base market | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Base price | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Target market | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| PPP index for base market | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| PPP index for target market | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional price-ending rule | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `RawPPPPrice = BasePrice × TargetPPPIndex / BasePPPIndex`
- `PPPAdjustment = RawPPPPrice / FXEquivalentPrice - 1 when FX equivalent supplied`
- `RoundedPrice = apply explicit selected price-ending rule`
...
- Raw PPP-equivalent price
- Rounded suggested price
- Difference vs base/FX price
- PPP adjustment percentage
...
- Verify `RawPPPPrice = BasePrice × TargetPPPIndex / BasePPPIndex` with at least one hand-calculated deterministic vector.
- Verify `PPPAdjustment = RawPPPPrice / FXEquivalentPrice - 1 when FX equivalent supplied` with at least one hand-calculated deterministic vector.
- Verify `RoundedPrice = apply explicit selected price-ending rule` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #08: PPP Price Calculator
...
- Base price
- Target market
- PPP index for base market
- PPP index for target market
- Optional price-ending rule
