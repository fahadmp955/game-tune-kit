# GameTuneKit Utility #14 — Loot / Drop-rate Calculator

## Part A — Product / Method Research

### Product question

> **Given a drop probability, how likely is a player to obtain an item after N attempts, and how many attempts are required for a target confidence?**
...
|---|---|---|---|
| Drop probability per attempt | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Number of attempts | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Target acquisition probability | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Cost per attempt | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
...
- `PZero = (1-p)^n`
- `ExpectedDrops = n×p`
- `AttemptsForTarget = ceil(log(1-target)/log(1-p))`
- `ExpectedCostToFirstDrop = CostPerAttempt / p`
...
- Verify `PAtLeastOne = 1 - (1-p)^n` with at least one hand-calculated deterministic vector.
- Verify `PZero = (1-p)^n` with at least one hand-calculated deterministic vector.
- Verify `ExpectedDrops = n×p` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #14: Loot / Drop-rate Calculator
