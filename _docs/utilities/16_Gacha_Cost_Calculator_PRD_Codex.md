# GameTuneKit Utility #16 — Gacha Cost Calculator

## Part A — Product / Method Research

### Product question

> **What does it cost, on average and at chosen confidence levels, to obtain a target item or complete a collection?**
...
|---|---|---|---|
| Pull price | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Target item probability | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Number of distinct collection items | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Duplicate policy | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional pity configuration | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Target confidence | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `Single target uses geometric distribution: P(success by n)=1-(1-p)^n`
- `AttemptsForConfidence = ceil(log(1-q)/log(1-p))`
- `ExpectedPullsSingle = 1/p`
- `ExpectedCost = pulls×pull price`
- `For equal-probability collection without pity use coupon-collector expectation E[N]=m×H_m; label as simplified`
...
- Expected pulls/cost
- Pulls/cost at target confidence
- Collection completion expectation
- Duplicate burden
...
- Acquisition probability curve
- Expected cost by target probability
...
- Verify `Single target uses geometric distribution: P(success by n)=1-(1-p)^n` with at least one hand-calculated deterministic vector.
- Verify `AttemptsForConfidence = ceil(log(1-q)/log(1-p))` with at least one hand-calculated deterministic vector.
- Verify `ExpectedPullsSingle = 1/p` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #16: Gacha Cost Calculator
...
- Pull price
- Target item probability
- Number of distinct collection items
- Duplicate policy
- Optional pity configuration
- Target confidence
