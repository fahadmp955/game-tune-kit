# GameTuneKit Utility #27 — A/B Test Sample Size Calculator

## Part A — Product / Method Research

### Product question

> **How many users and how many days does a standard two-variant experiment need to detect a target change?**
...
|---|---|---|---|
| Baseline conversion | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Minimum detectable effect | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Statistical power | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Significance level | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Daily eligible traffic | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Traffic split | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `Use standard two-proportion z-test sample-size approximation`
- `p2=p1×(1+MDE) for relative MDE mode`
- `n per variant = [zα√(2pbar(1-pbar))+zβ√(p1(1-p1)+p2(1-p2))]^2/(p2-p1)^2`
- `Duration=ceil(total required sample / daily eligible traffic) adjusted for split`
...
- Verify `Use standard two-proportion z-test sample-size approximation` with at least one hand-calculated deterministic vector.
- Verify `p2=p1×(1+MDE) for relative MDE mode` with at least one hand-calculated deterministic vector.
- Verify `n per variant = [zα√(2pbar(1-pbar))+zβ√(p1(1-p1)+p2(1-p2))]^2/(p2-p1)^2` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #27: A/B Test Sample Size Calculator
