# GameTuneKit Utility #33 — Churn Calculator

## Part A — Product / Method Research

### Product question

> **What share of a cohort or subscriber base churned, and what retention/lifetime does that imply under a constant-hazard assumption?**
...
|---|---|---|---|
| Starting users/subscribers | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Users lost | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Period length | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional ending users/new users | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `ChurnRate = Lost / Starting`
- `RetentionRate = 1-ChurnRate`
- `If period churn c and constant hazard: ExpectedPeriods≈1/c`
- `AnnualizedRetention=(1-c)^(periods/year)`
...
- Period churn
- Period retention
- Expected lifetime under constant churn
- Annualized retention/churn
...
- Verify `ChurnRate = Lost / Starting` with at least one hand-calculated deterministic vector.
- Verify `RetentionRate = 1-ChurnRate` with at least one hand-calculated deterministic vector.
- Verify `If period churn c and constant hazard: ExpectedPeriods≈1/c` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #33: Churn Calculator
...
- Starting users/subscribers
- Users lost
- Period length
- Optional ending users/new users
