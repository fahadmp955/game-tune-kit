# GameTuneKit Utility #35 — LiveOps Cadence Calculator

## Part A — Product / Method Research

### Product question

> **How dense is my LiveOps schedule, how much overlap does it create, and where are player-attention or operational-load collisions likely?**
...
|---|---|---|---|
| Events with start/end dates | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Event type | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Importance weight | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional target segment | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Maximum desired concurrent events | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `For each day: ConcurrentEvents=count(active events)`
- `WeightedLoad=sum(active event weights)`
- `OverlapDays=count(days concurrency>1)`
- `OverloadDays=count(days concurrency>desired max)`
- `CadenceGap=days between event starts`
...
- Verify `For each day: ConcurrentEvents=count(active events)` with at least one hand-calculated deterministic vector.
- Verify `WeightedLoad=sum(active event weights)` with at least one hand-calculated deterministic vector.
- Verify `OverlapDays=count(days concurrency>1)` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #35: LiveOps Cadence Calculator
...
- Maximum desired concurrent events
