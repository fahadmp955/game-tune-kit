# GameTuneKit Utility #10 — Currency Exchange Calculator

## Part A — Product / Method Research

### Product question

> **What is the effective exchange rate between two virtual currencies, and does a conversion path create arbitrage or hidden value leakage?**
...
|---|---|---|---|
| Source currency amount | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Target currency amount | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional reverse conversion | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Optional real-money anchor | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `ForwardRate = TargetAmount / SourceAmount`
- `ReverseImpliedRate = SourceAmount / TargetAmount`
- `RoundTripReturn = amount after forward and reverse / starting amount`
- `RoundTripLoss = 1 - RoundTripReturn`
...
- Inverse rate
- Round-trip loss
- Real-money equivalent when anchor supplied
...
- Verify `ForwardRate = TargetAmount / SourceAmount` with at least one hand-calculated deterministic vector.
- Verify `ReverseImpliedRate = SourceAmount / TargetAmount` with at least one hand-calculated deterministic vector.
- Verify `RoundTripReturn = amount after forward and reverse / starting amount` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #10: Currency Exchange Calculator
