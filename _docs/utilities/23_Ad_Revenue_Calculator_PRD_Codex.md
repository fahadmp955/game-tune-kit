# GameTuneKit Utility #23 — Ad Revenue Calculator

## Part A — Product / Method Research

### Product question

> **What ad revenue and ad ARPDAU should my audience generate from format, impression, fill-rate and eCPM assumptions?**
...
|---|---|---|---|
| DAU | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Impressions/user/day by format | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Fill rate by format | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| eCPM by format | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Days/month | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `ServedImpressions = DAU×RequestedImpressionsPerUser×FillRate`
- `DailyRevenue = sum(ServedImpressions×eCPM/1000)`
- `MonthlyRevenue=DailyRevenue×Days`
- `AdARPDAU=DailyRevenue/DAU`
...
- Revenue by format
- Revenue sensitivity to eCPM/fill
...
- Verify `ServedImpressions = DAU×RequestedImpressionsPerUser×FillRate` with at least one hand-calculated deterministic vector.
- Verify `DailyRevenue = sum(ServedImpressions×eCPM/1000)` with at least one hand-calculated deterministic vector.
- Verify `MonthlyRevenue=DailyRevenue×Days` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #23: Ad Revenue Calculator
...
- DAU
- Impressions/user/day by format
- Fill rate by format
- eCPM by format
- Days/month
