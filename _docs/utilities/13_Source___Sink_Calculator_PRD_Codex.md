# GameTuneKit Utility #13 — Source / Sink Calculator

## Part A — Product / Method Research

### Product question

> **Which economy sources and sinks dominate my currency flow, and is the configured economy balanced?**
...
|---|---|---|---|
| Named sources with amount/frequency | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Named sinks with amount/frequency | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| DAU or cohort size | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Time period | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `TotalSources = sum normalized source flows`
- `TotalSinks = sum normalized sink flows`
- `NetFlow = TotalSources - TotalSinks`
- `SinkCoverage = TotalSinks / TotalSources`
- `SourceShare_i = Source_i / TotalSources`
- `SinkShare_i = Sink_i / TotalSinks`
...
- Verify `TotalSources = sum normalized source flows` with at least one hand-calculated deterministic vector.
- Verify `TotalSinks = sum normalized sink flows` with at least one hand-calculated deterministic vector.
- Verify `NetFlow = TotalSources - TotalSinks` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #13: Source / Sink Calculator
...
- Named sources with amount/frequency
- Named sinks with amount/frequency
- DAU or cohort size
- Time period
