# GameTuneKit Utility #20 — Battle Pass Calculator

## Part A — Product / Method Research

### Product question

> **Does a Battle Pass contain enough progression and perceived value for its price and season length?**
...
|---|---|---|---|
| Pass price | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Season days | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Number of tiers | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| XP per tier / curve | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Expected daily XP | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Free-track reward value | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Premium-track reward value | User input | Required/optional as applicable | Validate non-negative/domain-specific values |
| Target completion percentile | User input | Required/optional as applicable | Validate non-negative/domain-specific values |

Use sensible UI defaults, but never imply that defaults are industry benchmarks.

## 5. Exact V1 calculation model

- `TotalXPRequired=sum tier XP`
- `RequiredXPPerDay=TotalXPRequired/SeasonDays`
- `ExpectedCompletionDays=TotalXPRequired/ExpectedDailyXP`
- `PremiumValueRatio=PremiumRewardValue/PassPrice`
- `TotalValueRatio=(Free+PremiumValue)/PassPrice`
...
- Verify `TotalXPRequired=sum tier XP` with at least one hand-calculated deterministic vector.
- Verify `RequiredXPPerDay=TotalXPRequired/SeasonDays` with at least one hand-calculated deterministic vector.
- Verify `ExpectedCompletionDays=TotalXPRequired/ExpectedDailyXP` with at least one hand-calculated deterministic vector.
...
```markdown
# CODEX TASK — Build GameTuneKit Utility #20: Battle Pass Calculator
...
- XP per tier / curve
- Expected daily XP
- Free-track reward value
- Premium-track reward value
- Target completion percentile
